...
abstract public class Symbol implements Cloneable {
    ...
    static public enum Types {
        INT, BOOL, CHAR, ARRAY, FUNCTION, PROCEDURE, VOID, STRING, UNDEFINED
    }
    ...
    public Symbol (String _name, Types _type) {
    	this (_name, -1, _type, ParameterClass.VAL, false); 
    }
    ...
    public static Symbol createNormalSymbol(String name, Symbol.Types type) {
		switch (type) {
			case INT:
				return new SymbolInt(name);
	
			case BOOL:
				return new SymbolBool(name);
	
			case CHAR:
				return new SymbolChar(name);
